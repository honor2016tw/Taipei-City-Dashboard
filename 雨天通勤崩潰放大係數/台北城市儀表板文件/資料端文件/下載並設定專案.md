...
### Step 1: 建立一個新的 Docker Image [#](#step-1:-建立一個新的-docker-image)

首先，打開你的終端，確保你位於包含 Dockerfile 的 `local-env` 目錄中。使用以下指令建立名為 `myairflow`, tag 為 `2.7.3` 的 Docker Image：

```
content_pastedocker build -t myairflow:2.7.3 .
```

### 第二步：創建 .env 文件 [#](#第二步：創建-.env-文件)

在當前目錄（docker-compose.yaml 所在目錄）中創建一個 `.env` 文件，並添加以下內容：

```
content_pasteAIRFLOW_IMAGE_NAME=myairflow:2.7.3
AIRFLOW_UID=50000
AIRFLOW_PROJ_DIR=../
...
```
content_pastedocker-compose ps
```

### 第四步：訪問 Airflow 網頁介面 [#](#第四步：訪問-airflow-網頁介面)
...
```
content_pasteConnection Id: postgres_default
...
```
content_pasteDEFAULT_EMAIL_LIST: ['your_email_1@mail', 'your_email_2@mail']
...
設置 PostgreSQL 數據庫 [#](#設置-postgresql-數據庫)
-----------------------------------------

要設置範例 PostgreSQL 環境，請按照以下步驟操作：

### 第一步：修改 .env 文件 [#](#第一步：修改-.env-文件)
...
### 第二步：啟動 PostgreSQL 和 pgAdmin 容器 [#](#第二步：啟動-postgresql-和-pgadmin-容器)
...
### 第三步：使用 pgAdmin 連接到 PostgreSQL [#](#第三步：使用-pgadmin-連接到-postgresql)
...
```
content_pasteHost name/address: dashboard-data
Port: 5432
...
###### 核心科技與必備知識](/documentation/data-end/prerequisites)
