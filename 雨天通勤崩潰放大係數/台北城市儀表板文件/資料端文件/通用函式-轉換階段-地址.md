...
### -函式 clean\_data(addr) [#](#-函式-clean_data(addr))
...
```
content_paste>>> print(ca)
0    台北市信義區三民路四段300號之1
1             信義路之8號5樓
dtype: object
```

### -函式 main\_process(addr\_cleaned) [#](#-函式-main_process(addr_cleaned))

辨識並切分地址，同時提供程式判斷信心等級。

### -函式 save\_data(addr, addr\_cleaned, standard\_addr\_list) [#](#-函式-save_data(addr,-addr_cleaned,-standard_addr_list))
...
```
content_paste>>> print(result.iloc[0])
status                                                 not null
conf_level                                              perfect
postcode
city                                                        台北市
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/data-end-ch/utils-transform-address.md)

[##### keyboard\_arrow\_left前一篇文章

###### 通用函式-轉換階段-空間](/documentation/data-end/utils-transform-spatial)
