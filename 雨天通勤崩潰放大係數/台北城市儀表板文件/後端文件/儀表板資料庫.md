...
```
content_pastetype Dashboard struct {
...
`dashboards` 表各欄位的詳細填寫方式（以確保前端相容性）可以在[這裡](/documentation/front-end/introduction-to-dashboards)找到。

dashboard\_groups [#](#dashboard_groups)
----------------------------------------

`PK` `FK` `dashboard_id` `group_id`

```
content_pastetype DashboardGroup struct {
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/back-end-ch/dashboards-db.md)
...
###### 問題回報資料庫](/documentation/back-end/issues-db)