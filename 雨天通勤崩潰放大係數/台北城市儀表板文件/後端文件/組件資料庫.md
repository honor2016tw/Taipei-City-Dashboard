...
###### Chart Config](https://www.youtube.com/watch?v=u0lUkc-NJ4E)[play\_circle

##### 組件地圖設定

###### Map Config](https://www.youtube.com/watch?v=08ynRI3dPvk)[play\_circle

##### 組件地圖篩選設定

###### Map Filter Config](https://www.youtube.com/watch?v=Tqh-ArEoYq4)[play\_circle
...
本專案使用四個表來儲存儀表板組件的配置。當這些表連接 (join) 起來時，將能組成前端所需的完整[組件配置](/documentation/front-end/introduction-to-components)。
...
```
content_pastetype Component struct {
...
`components` 表各欄位的詳細填寫方式（以確保前端相容性）可以在[這裡](/documentation/front-end/introduction-to-components)找到。

`v3.0.0`
因應多城市需求， 一個 Component (組件名稱) 可以有不同城市的 QueryCharts (組件資訊)，透過 index 做關聯

query\_charts [#](#query_charts)
--------------------------------

`FK` `index` `map_config_ids`

```
content_pastetype QueryCharts struct {
    Index          string          `json:"index"            gorm:"column:index;type:varchar;unique;not null"     `
...
`map_config_ids` 僅用於連接 `query_charts` 和 `component_maps` 表；`query_chart` 和 `query_history` 分別儲存用來檢索圖表和歷史資料的 SQL 指令。SQL 指令的相關撰寫指南可以在 [組件資料 API 文章](/documentation/back-end/component-data-apis)中找到。

`v3.0.0`
City 為必填欄位，目前可填寫的值為 `taipei` (台北), `metrotaipei` (雙北)

component\_charts [#](#component_charts)
----------------------------------------

`PK` `index`

```
content_pastetype ComponentChart struct {
...
`component_charts` 表各欄位的詳細填寫方式（以確保前端相容性）可以在[這裡](/documentation/front-end/supported-chart-types)找到。

component\_maps [#](#component_maps)
------------------------------------

`PK` `id`

```
content_pastetype ComponentMap struct {
    ID       int64           `json:"id"       gorm:"column:id;autoincrement;primaryKey"`
...
`component_maps` 表各欄位的詳細填寫方式（以確保前端相容性）可以在[這裡](/documentation/front-end/supported-map-types)找到。

[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/back-end-ch/components-db.md)
...
###### 儀表板資料庫](/documentation/back-end/dashboards-db)