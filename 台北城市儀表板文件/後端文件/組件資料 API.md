...
###### Chart and History Data](https://www.youtube.com/watch?v=1OARa32Ay3o)
...
任何 `dashboard` 資料庫可以執行的 SQL 指令均可以被使用。此外，SQL 指令還可以透過佔位符（`%s`）挖空，如果在[組件配置](/documentation/front-end/introduction-to-components)中有指定組件時間區間，這些占位符將被替換成指定的時間範圍，以檢索特定時間範圍內的資料。占位符的值會由前端自動生成。

圖表資料的 SQL 指令可以有 0 或 2 個占位符。如果 SQL 指令有 2 個占位符，檢索將回傳指定時間範圍的資料。

至於歷史資料，檢索類型 (query\_type) 均為 `time`，且 SQL 指令必須有 3 個占位符。第一個占位符是時間單位（例如 `day`，`week`，`month`，根據開始和結束時間自動計算），第二個占位符是開始時間，第三個占位符是結束時間。

檢索類型 (Query Type) [#](#檢索類型-(query-type))
-----------------------------------------

> #### 小撇步 - 1
>
> 我們建議您在閱讀此部分之前先熟悉前端文件中的[圖表資料文章](/documentation/front-end/chart-data)。
...
```
content_pastetype TwoDimensionalData struct {
...
```
content_paste-- '臺北市陳情系統 circular'
...
```
content_pastetype ThreeDimensionalData struct {
...
```
content_paste-- '全市屋齡分布 building_age'
...
`percent` 檢索類型與 `three_d` 檢索類型的格式相同，但對於資料意義有一些額外的限制。更多資訊請參考前端文件中的[圖表資料文章](/documentation/front-end/chart-data)。

**範例 SQL 指令：**

```
content_paste-- 'YouBike使用情況 youbike_availability'
SELECT
...
```
content_pastetype TimeSeriesData struct {
...
```
content_paste-- '建照使照發照量 building_license'
WITH date_range AS (
  SELECT
...
```
content_paste-- '捷運人流趨勢 metro_passengers'
...
### map\_legend [#](#map_legend)

**目標檢索結果：**

```
content_pastetype MapLegendData struct {
...
```
content_paste-- '居職人口推估 fet_work_live'
SELECT
...
| --- | --- |
| 權限 | `Guest` |
| 查詢參數 | `timefrom` ---- 查詢的開始時間。可選。 `timeto` -------- 查詢的結束時間。可選。 |
...
| --- | --- |
| 權限 | `Guest` |
| 查詢參數 | `timefrom` ---- 查詢的開始時間。必填。 `timeto` -------- 查詢的結束時間。必填。 |
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/back-end-ch/component-data-apis.md)
