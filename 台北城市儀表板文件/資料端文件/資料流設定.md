...
在[撰寫Python程式碼](/documentation/data-end/dag-code)的宣告變數部分使用了許多動態變數，變數的來源除了全域共用的變數外，有一大部分為每個 DAG 獨立的設定檔。
...
任意 e-mail 地址，或以 `[]` 包裝的多個 e-mail 地址，也可以是 "\_MAIL\_LIST" 結尾的變數。例如 `["test@mail", "DEFAULT_MAIL_LIST"]`，其中的 `"DEFAULT_MAIL_LIST"` 會自動尋找 Airflow Variables符合的 Key，並取用其中的 Value。使開發者可以靈活地建立 mail 群組使用。
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/data-end-ch/dag-config.md)

[##### keyboard\_arrow\_left前一篇文章

###### 資料流程式碼](/documentation/data-end/dag-code)

[##### 下一篇文章keyboard\_arrow\_right

###### 資料流表格](/documentation/data-end/dag-table)