...
### -函式 get\_data\_taipei\_api(rid, output\_format) [#](#-函式-get_data_taipei_api(rid,-output_format))
...
```
content_pastefrom utils.extract_stage import get_data_taipei_api
...
### get\_kml(url, dag\_id, from\_crs) [#](#get_kml(url,-dag_id,-from_crs))

下載並讀取 `url` 的 kml 檔案並回傳 gpd.GeoDataFrame。

```
content_pastefrom utils.extract_stage import get_kml
...
### -函式 get\_shp\_file(url, dag\_id, from\_crs) [#](#-函式-get_shp_file(url,-dag_id,-from_crs))

下載 `url` 的壓縮檔，並讀取其中的第一個 .shp 檔案，並回傳 gpd.GeoDataFrame。範例如下：

```
content_pastefrom utils.extract_stage import get_shp_file
...
```
content_paste>>> print(raw_data.iloc[0])
...
### -函式 get\_moenv\_json\_data(dataset\_code) [#](#-函式-get_moenv_json_data(dataset_code))

取得環境開放資料平台 API 資料回傳 JSON 格式。範例如下：

```
content_pastetemp = get_moenv_json_data("AQX_P_136", is_test=True)
print(temp[0])
```

```
content_paste>>> print(temp[0])
...
### -函式 get\_tdx\_data(url, output\_format) [#](#-函式-get_tdx_data(url,-output_format))

取得 TDX 平臺 API 資料，回傳 `output_format` 格式。範例如下：

```
content_pastefrom utils.extract_stage import get_tdx_data
...
### -函式 get\_json\_file(url, dag\_id, output\_format) [#](#-函式-get_json_file(url,-dag_id,-output_format))

下載並讀取 `url` 的 JSON 檔案，回傳 `output_format` 格式。範例如下：

```
content_pastefrom utils.extract_stage import get_json_file
...
### -函式 get\_geojson\_file(url, dag\_id, from\_crs) [#](#-函式-get_geojson_file(url,-dag_id,-from_crs))

下載並讀取 `url` 的 GeoJSON 資料，並回傳 gpd.GeoDataFrame。範例如下：

```
content_pastefrom utils.extract_stage import get_geojson_file
...
### -函式 get\_data\_taipei\_file\_last\_modified\_time(page\_id, rank) [#](#-函式-get_data_taipei_file_last_modified_time(page_id,-rank))

取得 data.taipei 指定 `page_id` 頁面的檔案更新時間，通常在頁面下方，下載資料按鈕的左邊。一個頁面可能有多個檔案與更新時間，可使用 `rank` 參數取得指定位置（0 表示最上面）。

```
content_pastefrom utils.extract_stage import get_data_taipei_file_last_modified_time
...
```
content_paste>>> print(res)
'2023-06-06 09:53:08'
```

### -函式 get\_data\_taipei\_page\_change\_time(page\_id, rank) [#](#-函式-get_data_taipei_page_change_time(page_id,-rank))

取得 data.taipei 指定 `page_id` 頁面的異動時間，通常在頁面左方頁籤。一個頁面可能有多個異動時間，可使用 `rank` 參數取得指定位置（0 表示最上面）。

```
content_pastefrom utils.extract_stage import get_data_taipei_page_change_time
...
### -函式 download\_file(file\_name, url) [#](#-函式-download_file(file_name,-url))

從 `url` 下載檔案儲存於 `./DATA_PATH/{file_name}`，並回傳下載檔案的完整路徑。範例如下：

```
content_paste# Read GeoJSON
...
### -函式 unzip\_file\_to\_target\_folder(zip\_file, unzip\_path) [#](#-函式-unzip_file_to_target_folder(zip_file,-unzip_path))

將 `zip_file` 解壓縮至 `unzip_path` 資料夾。

```
content_paste# Read Shapefile
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/data-end-ch/utils-extract.md)

[##### keyboard\_arrow\_left前一篇文章

###### 通用函式-建表](/documentation/data-end/utils-generate-sql)

[##### 下一篇文章keyboard\_arrow\_right

###### 通用函式-轉換階段-時間](/documentation/data-end/utils-transform-time)