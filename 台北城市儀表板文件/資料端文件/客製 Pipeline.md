...
如[撰寫Python程式碼](/documentation/data-end/dag-code)章節所示，本專案使用自製的 pipeline 來隱藏程式與 Airflow 互動的細節。本章將說明我們所使用的 `common_pipeline` 的設計理念與程式。

標準的 Airflow DAG [#](#標準的-airflow-dag)
-------------------------------------
...
由於實際程式碼篇幅較長，請至 `dags\operators\common_pipeline.py` 查看原始碼，或查看 [GitHub連結](/documentation)。為了方便理解，說明將從程式碼下方往上閱讀。

此 `pipeline` 總共有四個 `task` 如 246 行所示：

```
content_pasteget_and_validate_config >> etl >> update_dataset_info >> dag_execution_success
```

looks\_one task `get_and_validate_config`，讀取DAG設定檔，也就是前面文件所教學的 `jon_config.json` 內容。

looks\_two task `etl` 代入各種設定，實際執行資料處理。此步驟所執行的內容並不在 `common_pipeline.py`，而是[撰寫Python程式碼](/documentation/data-end/dag-code)章節所示範的，一般情況所需開發的每個 DAG 獨立的 ETL 程式碼。

looks\_3 task `update_dataset_info` 將 `jon_config.json` 中的 `data_infos` 部分更新至 `db:dashboard/dashboard/dataset_info`。

looks\_4 task `dag_execution_success` 僅用來表示 DAG 已完成，無任何作用。

這四個 `task` 的 `operator` 定義於 222~243 行：

```
content_paste        with dag:
...
```
content_paste    def create_dag(self, etl_func=_etl_func_missing):
...
```
content_pastedef _read_config(path, file_name='job_config.json'):
    ...
    return config_dict
...
```
content_pastedef _create_or_update_dataset_info(psql_uri, config):
    ...
    conn.close()
```
...
```
content_pastedef _etl_func_missing():
    ...
...
```
content_paste    def fetch_email_list(self, mail_list: list):
        ...
...
[auto\_fix\_high於Github編輯此文章](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/data-end-ch/pipeline.md)

[##### keyboard\_arrow\_left前一篇文章

###### 資料流測試](/documentation/data-end/dag-test)

[##### 下一篇文章keyboard\_arrow\_right

###### 通用函式概覽](/documentation/data-end/utils-overview)