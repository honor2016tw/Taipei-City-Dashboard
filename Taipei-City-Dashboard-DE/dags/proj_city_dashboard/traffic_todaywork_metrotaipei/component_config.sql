...
VALUES ('traffic_todaywork', ARRAY['#2f9ca1'], ARRAY['DistrictChart', 'ColumnChart'], '件')
ON CONFLICT ("index") DO NOTHING;

-- 若 component_charts 列已存在（ON CONFLICT DO NOTHING 不更新），用 UPDATE 確保圖表類型正確
UPDATE public.component_charts
SET "types" = ARRAY['DistrictChart', 'ColumnChart']
...
VALUES (
    ...
    '整合臺北市今日施工資訊與新北市道路挖掘案件資料，統計目前雙北各行政區的施工案件數。臺北市資料取自臺北市政府工務局 Todaywork.json；新北市資料取自新北市政府開放資料「新北市政府道路挖掘資訊」CSV API，並以案件施工起訖日期判斷今日仍有效的案件。圖表可切換行政區圖與縱向長條圖，用於快速掌握施工熱區與跨市通勤路廊可能受影響的位置。',
...
-- ── 台北市地圖圖層（新北透明，只顯示臺北市施工點）──────────────────────────────
DELETE FROM public.component_maps WHERE "id" = 20021;
...
VALUES (
    ...
    'big',
    NULL,
    '{"circle-color": "#2f9ca1", "circle-radius": ["interpolate", ["linear"], ["zoom"], 10, 4, 13, 7, 16, 11], "circle-opacity": ["match", ["get", "source_city"], "臺北市", 0.82, 0], "circle-stroke-color": "#ffffff", "circle-stroke-width": 1}'::json,
    '[{"key":"source_city","name":"縣市"},{"key":"district","name":"行政區"},{"key":"project_name","name":"工程名稱"},{"key":"organizer","name":"主辦單位"},{"key":"address","name":"施工位置"},{"key":"date_range","name":"施工日期"},{"key":"source","name":"資料來源"}]'::json
);
...
VALUES (
    ...
    '臺北市政府工務局',
    '顯示目前臺北市道路施工案件的行政區分布。',
    '整合臺北市今日施工資訊，統計各行政區的施工案件數。資料取自臺北市政府工務局 Todaywork.json。圖表可切換行政區圖與縱向長條圖，用於快速掌握施工熱區。',
    '通勤者可在出門前查看臺北哪些行政區施工案件較多，避開施工密集區域；交通或工務單位可用來比較各行政區施工量，作為道路施工告示與交通疏導的參考。',
    ARRAY[
        'https://data.taipei/dataset/detail?id=c208dabd-2da0-4e6d-8dbd-a004b9782b0a'
    ],
    ARRAY['tuic'],
